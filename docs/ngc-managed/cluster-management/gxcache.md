...
<details>
<summary>BYOC (NGC Helm Repo)</summary>

</details>

```bash
 helm repo add nvcf-byoc \
   https://helm.ngc.nvidia.com/nvidia/nvcf-byoc \
   --username '$oauthtoken' --password "${NGC_API_KEY}" --force-update
 helm repo update
```

<details>
<summary>Self-Hosted (NGC OCI)</summary>

</details>

```bash
 echo "${NGC_API_KEY}" | helm registry login nvcr.io \
   --username '$oauthtoken' --password-stdin
```

<details>
<summary>Self-Hosted (Amazon ECR)</summary>

</details>

```bash
 aws ecr get-login-password --region <region> | \
   helm registry login <account-id>.dkr.ecr.<region>.amazonaws.com \
...
<details>
<summary>Self-Hosted (OCI)</summary>
...
```yaml
 webhook:
   nodeSelector:
     nvcf.nvidia.com/workload: gpu  # Adjust based on your node labels, or set to null
   deployment:
 ...
 service:
   ...
   kns:
     enabled: true
     keyset:
       endpoint: https://notary-service.nvcf.nvidia.com
...
```yaml
 ...
 webhook:
   nodeSelector:
     nvcf.nvidia.com/workload: gpu  # Adjust based on your node labels
   deployment:
     image: <your-registry>/<your-repo>/gxcache-webhook
     version: 59bd8ec5
