...
# create default service paths and secrets under /services/sis/kv
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "cassandra/creds" "username=sis_api_app_v0 password=${DEFAULT_CASSANDRA_PASSWORD} tls_cert=replace public_key=replace private_key=replace"

# secrets below should be removed at some point in the future
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "lightstep" "access_token=replace"
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "ssa_client_credentials" "ssa_client_id=replace ssa_client_secret=replace"
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "metrics" "key=replace"
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "credential" "key=replace"
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "oci_k8s_credentials" "api_token=replace ssl_cert=replace"
