...
SERVICE_ACCOUNT_NAME="invocation-api"
...
#-------------------------------------------
write_secrets_kv "${VAULT_SECRET_BASE_PATH}/kv" "nats/api_auth" "worker_seed=replace"
...
SCOPES="invocation:check_invocation"
...
# Create policy for JWT Secret Role
#-------------------------------------------
policy=$(generate_acl_policy "${RATELIMITER_API_SECRET_BASE_PATH}/jwt/sign/${SERVICE_ACCOUNT_NAME}" "create,update,read")
policy_name="${RATELIMITER_API_SECRET_POLICY_PATH}-jwt-sign-${SERVICE_ACCOUNT_NAME}-rw"
write_policy "${policy_name}" "${policy}"
...
if ! create_auth_jwt_role "${SERVICE_ACCOUNT_NAME}" "${jwt_auth_role}"; then
    log_error "Failed to create JWT auth role for ${SERVICE_ACCOUNT_NAME}"
    exit 1
fi
