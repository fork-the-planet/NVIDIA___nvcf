...
def _go_oci_image_impl(name, visibility, binary, base, entrypoint, registry, tags):
    ...
    # mode = "0755" is defensive: a Go binary out of rules_go is already
    # 0755 on disk and pkg_tar preserves that, so the layer is normally
    # fine without this. But if any future caller passes a non-Go file
    # (an embedded data file, a wrapper script generated by a genrule
    # without explicit chmod), pkg_tar would default it to mode 0644 and
    # the resulting container would crash with "permission denied" --
