...
const (
	EnvVaultAddress          = "ESS_ADDR"
	EnvVaultAgentAddr        = "ESS_AGENT_ADDR"
	EnvVaultCACert           = "ESS_CACERT"
	EnvVaultCACertBytes      = "ESS_CACERT_BYTES"
	EnvVaultCAPath           = "ESS_CAPATH"
	EnvVaultClientCert       = "ESS_CLIENT_CERT"
	EnvVaultClientKey        = "ESS_CLIENT_KEY"
	EnvVaultClientTimeout    = "ESS_CLIENT_TIMEOUT"
	EnvVaultSRVLookup        = "ESS_SRV_LOOKUP"
	EnvVaultSkipVerify       = "ESS_SKIP_VERIFY"
	EnvVaultNamespace        = "ESS_NAMESPACE"
	EnvVaultTLSServerName    = "ESS_TLS_SERVER_NAME"
	EnvVaultWrapTTL          = "ESS_WRAP_TTL"
	EnvVaultMaxRetries       = "ESS_MAX_RETRIES"
	EnvVaultToken            = "ESS_TOKEN"
	EnvVaultMFA              = "ESS_MFA"
	EnvRateLimit             = "ESS_RATE_LIMIT"
	EnvHTTPProxy             = "ESS_HTTP_PROXY"
	EnvVaultProxyAddr        = "ESS_PROXY_ADDR"
	EnvVaultDisableRedirects = "ESS_DISABLE_REDIRECTS"
	HeaderIndex              = "X-ESS-Index"
	HeaderForward            = "X-ESS-Forward"
	HeaderInconsistent       = "X-ESS-Inconsistent"
	TLSErrorString           = "This error usually means that the server is running with TLS disabled\n" +
		"but the client is configured to use TLS. Please either enable TLS\n" +
		"on the server or run the client with -address set to an address\n" +
		"that uses the http protocol:\n\n" +
		"    vault <command> -address http://<address>\n\n" +
		"You can also set the ESS_ADDR environment variable:\n\n\n" +
		"    ESS_ADDR=http://<address> vault <command>\n\n" +
		"where <address> is replaced by the actual address to the server."
)

// Deprecated values
const (
	EnvVaultAgentAddress = "ESS_AGENT_ADDR"
	EnvVaultInsecure     = "ESS_SKIP_VERIFY"
)
...
// setting the `ESS_ADDR` environment variable.
//
...
func (c *Config) ReadEnvironment() error {
	...
	// ESS_PROXY_ADDR supersedes ESS_HTTP_PROXY
...
//
// If the environment variable `ESS_TOKEN` is present, the token will be
...
// value of ESS_ADDR environment variable.
...
func (c *Client) setNamespace(namespace string) {
	...
	// added for nv
	if IsLocalDev() {
		c.headers.Set(HttpHeaderVaultNamespace, namespace)
	}
}
...
START:
	...
	// added for ess
	LogRequestId(ns, result, hclog.Default())
...
func (c *Client) httpRequestWithContext(ctx context.Context, r *Request) (*Response, error) {
	...
	if len(r.ClientToken) != 0 {
		...
		// added for nv
		ShimVaultTokenHeader(req, r.ClientToken)
	}
...
// 412 requests, which are returned by Vault when a X-ESS-Index header isn't
// satisfied.
