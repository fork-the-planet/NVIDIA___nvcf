...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand, packaging, and telemetry or otherwise documented as
...
func (cli *CLI) Run(args []string) int {
	...
	// added for ess
	// add uuid to ess agent's HTTP requests via headers
	agentUUID, err := uuid.GenerateUUID()
	if err != nil {
		log.Fatalf("could not generate uuid for agent - error: %v", err)
		return 1
	}
	agentUUID = strings.Replace(agentUUID, "-", "", -1)
	fmt.Fprintf(cli.outStream, "ESS Agent instance id: %s\n", agentUUID)
	fmt.Fprintf(cli.outStream, "ESS Agent init(mode): %t\n", nv.IsInInitMode())
	if config.ESSHeaders == nil {
		config.ESSHeaders = map[string]string{
			api.HttpHeaderNameEssAgentID: agentUUID,
		}
	} else {
		config.ESSHeaders[api.HttpHeaderNameEssAgentID] = agentUUID
	}

	var tm *telemetry.Telemetry
	if !nv.IsInInitMode() {
		// Initialize telemetry on non-init container usage
		tm, err = telemetry.New(telemetry.TelemetryParams{
			C: config.Telemetry,
			Tags: map[string]string{
				"ess_agent_id": agentUUID,
			}})
		if err != nil {
			return logError(err, ExitCodeConfigError)
		}
	...
	// Initial runner
	runner, err := manager.NewRunner(config, dry, tm)
	if err != nil {
...
func (cli *CLI) ParseFlags(args []string) (
	*config.Config, []string, bool, bool, error,
	...
	flags.Var((funcVar)(func(s string) error {
		c.ESS.Address = config.String(s)
		return nil
	}), "ess-addr", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.RenewToken = config.Bool(b)
		return nil
	}), "ess-renew-token", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.Retry.Enabled = config.Bool(b)
		return nil
	}), "ess-retry", "")

	flags.Var((funcIntVar)(func(i int) error {
		c.ESS.Retry.Attempts = config.Int(i)
		return nil
	}), "ess-retry-attempts", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.Retry.Backoff = config.TimeDuration(d)
		return nil
	}), "ess-retry-backoff", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.Retry.MaxBackoff = config.TimeDuration(d)
		return nil
	}), "ess-retry-max-backoff", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.ClientUserAgent = config.String(s)
		return nil
	}), "ess-client-user-agent", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.SSL.Enabled = config.Bool(b)
		return nil
	}), "ess-ssl", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.SSL.CaCert = config.String(s)
		return nil
	}), "ess-ssl-ca-cert", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.SSL.CaPath = config.String(s)
		return nil
	}), "ess-ssl-ca-path", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.SSL.Cert = config.String(s)
		return nil
	}), "ess-ssl-cert", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.SSL.Key = config.String(s)
		return nil
	}), "ess-ssl-key", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.SSL.ServerName = config.String(s)
		return nil
	}), "ess-ssl-server-name", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.SSL.Verify = config.Bool(b)
		return nil
	}), "ess-ssl-verify", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.Transport.DialKeepAlive = config.TimeDuration(d)
		return nil
	}), "ess-transport-dial-keep-alive", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.Transport.DialTimeout = config.TimeDuration(d)
		return nil
	}), "ess-transport-dial-timeout", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.Transport.DisableKeepAlives = config.Bool(b)
		return nil
	}), "ess-transport-disable-keep-alives", "")

	flags.Var((funcIntVar)(func(i int) error {
		c.ESS.Transport.MaxIdleConnsPerHost = config.Int(i)
		return nil
	}), "ess-transport-max-idle-conns-per-host", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.Transport.TLSHandshakeTimeout = config.TimeDuration(d)
		return nil
	}), "ess-transport-tls-handshake-timeout", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.Token = config.String(s)
		return nil
	}), "ess-token", "")

	flags.Var((funcVar)(func(s string) error {
		c.ESS.VaultAgentTokenFile = config.String(s)
		return nil
	}), "ess-agent-token-file", "")

	flags.Var((funcBoolVar)(func(b bool) error {
		c.ESS.UnwrapToken = config.Bool(b)
		return nil
	}), "ess-unwrap-token", "")

	flags.Var((funcDurationVar)(func(d time.Duration) error {
		c.ESS.DefaultLeaseDuration = config.TimeDuration(d)
		return nil
	}), "ess-default-lease-duration", "")
...
Options:
  ...
  -ess-addr=<address>
      Sets the address of the ESS server

  -ess-renew-token
      Periodically renew the provided ESS API token - this defaults to "true"
      and will renew the token at half of the lease duration

  -ess-retry
      Use retry logic when communication with ESS fails

  -ess-retry-attempts=<int>
      The number of attempts to use when retrying failed communications

  -ess-retry-backoff=<duration>
      The base amount to use for the backoff duration. This number will be
      increased exponentially for each retry attempt.

  -ess-retry-max-backoff=<duration>
      The maximum limit of the retry backoff duration. Default is one minute.
      0 means infinite. The backoff will increase exponentially until given value.

  -ess-ssl
      Specifies whether communications with ESS should be done via SSL

  -ess-ssl-ca-cert=<string>
      Sets the path to the CA certificate to use for TLS verification

  -ess-ssl-ca-path=<string>
      Sets the path to the CA to use for TLS verification
  ...
  -ess-ssl-server-name=<string>
      Sets the name of the server to use when validating TLS.

  -ess-ssl-verify
      Enable SSL verification for communications with ESS.

  -ess-token=<token>
      Sets the ESS API token

  -ess-agent-token-file=<token-file>
      File to read ESS API token from.

  -ess-transport-dial-keep-alive=<duration>
      Sets the amount of time to use for keep-alives

  -ess-transport-dial-timeout=<duration>
      Sets the amount of time to wait to establish a connection

  -ess-transport-disable-keep-alives
      Disables keep-alives (this will impact performance)

  -ess-transport-max-idle-conns-per-host=<int>
      Sets the maximum number of idle connections to permit per host

  -ess-transport-tls-handshake-timeout=<duration>
      Sets the handshake timeout

  -ess-unwrap-token
      Unwrap the provided ESS API token (see ESS documentation for more
      information on this feature)

  -ess-default-lease-duration=<duration>
