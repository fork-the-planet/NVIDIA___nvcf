...
func TestCLI_ParseFlags(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"ess-addr",
			[]string{"-ess-addr", "vault_addr"},
			&config.Config{
				ESS: &config.VaultConfig{
		...
		},
		{
			...
			[]string{"-ess-retry-max-backoff", "60s"},
			&config.Config{
				ESS: &config.VaultConfig{
					Retry: &config.RetryConfig{
						MaxBackoff: config.TimeDuration(60 * time.Second),
		...
		{
			"ess-ssl-ca-cert",
			[]string{"-ess-ssl-ca-cert", "ca_cert"},
			&config.Config{
				ESS: &config.VaultConfig{
					SSL: &config.SSLConfig{
						CaCert: config.String("ca_cert"),
		...
		{
			"ess-ssl-ca-path",
			[]string{"-ess-ssl-ca-path", "ca_path"},
			&config.Config{
				ESS: &config.VaultConfig{
					SSL: &config.SSLConfig{
						CaPath: config.String("ca_path"),
		...
		{
			"ess-ssl-server-name",
			[]string{"-ess-ssl-server-name", "server_name"},
			&config.Config{
				ESS: &config.VaultConfig{
					SSL: &config.SSLConfig{
						ServerName: config.String("server_name"),
		...
		},
		{
			...
			[]string{"-ess-agent-token-file", "/tmp/vault/agent/token"},
			&config.Config{
				ESS: &config.VaultConfig{
		...
		},
		{
			...
			[]string{"-ess-transport-dial-keep-alive", "30s"},
			&config.Config{
				ESS: &config.VaultConfig{
					Transport: &config.TransportConfig{
						DialKeepAlive: config.TimeDuration(30 * time.Second),
		...
		},
		{
			...
			[]string{"-ess-transport-dial-timeout", "30s"},
			&config.Config{
				ESS: &config.VaultConfig{
					Transport: &config.TransportConfig{
						DialTimeout: config.TimeDuration(30 * time.Second),
		...
		},
		{
			...
			[]string{"-ess-transport-disable-keep-alives"},
			&config.Config{
				ESS: &config.VaultConfig{
					Transport: &config.TransportConfig{
						DisableKeepAlives: config.Bool(true),
		...
		},
		{
			...
			[]string{"-ess-transport-max-idle-conns-per-host", "100"},
			&config.Config{
				ESS: &config.VaultConfig{
					Transport: &config.TransportConfig{
						MaxIdleConnsPerHost: config.Int(100),
		...
		},
		{
			...
			[]string{"-ess-transport-tls-handshake-timeout", "30s"},
			&config.Config{
				ESS: &config.VaultConfig{
					Transport: &config.TransportConfig{
						TLSHandshakeTimeout: config.TimeDuration(30 * time.Second),
		...
		},
		{
			...
			[]string{"-ess-default-lease-duration", "60s"},
			&config.Config{
				ESS: &config.VaultConfig{
...
func TestCLI_Run(t *testing.T) {
	...
	t.Run("once", func(t *testing.T) {
		if os.Getenv("SKIP_INTEGRATION_TESTS") != "" {
			t.Skip("requires consul — see KNOWN_TEST_ISSUES.md")
		}
		f, err := os.CreateTemp("", "")
		if err != nil {
			t.Fatal(err)
	...
	t.Run("once_from_env", func(t *testing.T) {
		if os.Getenv("SKIP_INTEGRATION_TESTS") != "" {
			t.Skip("requires consul — see KNOWN_TEST_ISSUES.md")
		}
		os.Setenv(nv.EnvESSAgentInit, "true")
		defer func() {
			os.Unsetenv(nv.EnvESSAgentInit)
		}()
...
func TestNV_CLIRun(t *testing.T) {
	t.Run("kill_signal_sigterm_default", func(t *testing.T) {
		out := gatedio.NewByteBuffer()
		cli := NewCLI(out, out)
		defer cli.stop()
		...
		// wait a moment for the CLI to start
		time.Sleep(100 * time.Millisecond)

		// end SIGTERM signal (should trigger graceful shutdown)
		cli.signalCh <- syscall.SIGTERM

		select {
		case status := <-ch:
			if status != ExitCodeInterrupt {
				t.Errorf("expected ExitCodeInterrupt (%d), got %d", ExitCodeInterrupt, status)
			}
			// verify the output contains cleanup message
			output := out.String()
			if !strings.Contains(output, "Cleaning up...") {
				t.Errorf("expected 'Cleaning up...' in output, got: %s", output)
			}
		case <-time.After(2 * time.Second):
			t.Errorf("timeout waiting for SIGTERM handling: %q", out.String())
		}
	})

	t.Run("kill_signal_sigint_default_fails", func(t *testing.T) {
		out := gatedio.NewByteBuffer()
		cli := NewCLI(out, out)
		defer cli.stop()
		...
		// end SIGINT signal (should trigger graceful shutdown)
		cli.signalCh <- syscall.SIGINT
		...
		case <-time.After(2 * time.Second):
			t.Logf("timeout waiting for SIGINT handling: %q", out.String())
		}
	})

	t.Run("kill_signal_sigint_default_changed_success", func(t *testing.T) {
		out := gatedio.NewByteBuffer()
		cli := NewCLI(out, out)
		defer cli.stop()
