...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand, telemetry, and SIGTERM shutdown behavior or otherwise documented as
...
func TestMain(m *testing.M) {
	...
	os.Unsetenv("ESS_ADDR")
	os.Unsetenv("ESS_TOKEN")
	os.Unsetenv("ESS_DEV_ROOT_TOKEN_ID")
...
func TestParse(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"ess_agent_token_file",
			`ess {
				ess_agent_token_file = "` + testFilePath + `"
			}`,
			&Config{
				ESS: &VaultConfig{
...
func TestFinalize(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			"ess_agent_token_file",
			func(act, exp *Config) (bool, error) {
				actToken := *act.ESS.Token
				expToken := *exp.ESS.Token
				if actToken != expToken {
					return false, fmt.Errorf("tokens don't match: %v != %v",
						actToken, expToken)
				}
				actTokenFile := *act.ESS.VaultAgentTokenFile
				expTokenFile := *exp.ESS.VaultAgentTokenFile
...
func TestDefaultConfig(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			&Config{
				ESS: &VaultConfig{
					K8SAuthRoleName: String("ESS_K8S_AUTH_ROLE_NAME"),
				},
			},
			false,
		},
		{
			...
			&Config{
				ESS: &VaultConfig{
					K8SServiceAccountToken: String("ESS_K8S_SERVICE_ACCOUNT_TOKEN"),
				},
			},
			false,
		},
		{
			...
			&Config{
				ESS: &VaultConfig{
					K8SServiceAccountTokenPath: String("ESS_K8S_SERVICE_ACCOUNT_TOKEN_PATH"),
				},
			},
			false,
		},
		{
			...
			&Config{
				ESS: &VaultConfig{
					K8SServiceMountPath: String("ESS_K8S_SERVICE_MOUNT_PATH"),
				},
			},
			false,
		},
