...
type TemplateConfig struct {
	...
	// added for nv
	// User specifies which user should own the rendered file
	// nv TODO remove after validating open source implementation of file ownership
	//User *string `mapstructure:"user"`

	// added for nv
	// Group specifies which group should own the rendered file
	// nv TODO remove after validating open source implementation of file ownership
	//Group *string `mapstructure:"group"`

	// added for nv
	// RenderOnce determines if a template is to be rendered only one time
	RenderOnce *bool `mapstructure:"render_once"`
	...
	SkipMountVersionCheck *bool `mapstructure:"skip_mount_version_check"`

	// added for nv
	// ExitOnClientError determines if process should exit if the secret path(s) defined in the template
	// return a 40x status code
	ExitOnClientError *bool `mapstructure:"exit_on_client_error"`
	...
	StopProcessingOnClientError *bool `mapstructure:"stop_processing_on_client_error"`
}
...
func (c *TemplateConfig) Copy() *TemplateConfig {
	...
	o.User = c.User
	o.Group = c.Group
	o.RenderOnce = c.RenderOnce
	o.SkipMountVersionCheck = c.SkipMountVersionCheck
	o.ExitOnClientError = c.ExitOnClientError
	o.StopProcessingOnClientError = c.StopProcessingOnClientError
...
func (c *TemplateConfig) Merge(o *TemplateConfig) *TemplateConfig {
	...
	// start for nv
	if o.RenderOnce != nil {
		r.RenderOnce = o.RenderOnce
	}
	if o.SkipMountVersionCheck != nil {
		r.SkipMountVersionCheck = o.SkipMountVersionCheck
	}

	if o.ExitOnClientError != nil {
		r.ExitOnClientError = o.ExitOnClientError
	}

	if o.StopProcessingOnClientError != nil {
		r.StopProcessingOnClientError = o.StopProcessingOnClientError
	}
	// end for nv
...
func (c *TemplateConfig) Finalize() {
	...
	if c.ErrMissingKey == nil {
		// added for nv
		// update the default to true
		c.ErrMissingKey = Bool(true)
	}

	// added for nv
	// for ESS Agent if exit_on_client_err is not set, the default to true.
	if c.ExitOnClientError == nil {
		c.ExitOnClientError = Bool(true)
	}

	// added for nv
	// for ESS Agent if stop_processing_on_client_error is not set, the default to false.
	if c.StopProcessingOnClientError == nil {
		c.StopProcessingOnClientError = Bool(false)
	}
