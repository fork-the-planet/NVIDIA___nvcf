...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand and Vault template behavior or otherwise documented as
...
const (
	...
	DefaultVaultLeaseDuration = 15 * time.Minute
...
type VaultConfig struct {
	...
	// also be set via the ESS_NAMESPACE environment variable.
	...
	// a wrapped token or a real token. This can also be set via the ESS_TOKEN
	...
	// If ess_agent_token_file is specified:
	...
	// changed.
	VaultAgentTokenFile *string `mapstructure:"ess_agent_token_file" json:"-"`
	...
	// a Kubernetes Pod.
	//
	// This can also be set via the ESS_K8S_AUTH_ROLE_NAME.
	...
	//
	// This can also be set via the ESS_K8S_SERVICE_ACCOUNT_TOKEN_PATH.
	...
	//
	// This can also be set via the ESS_K8S_SERVICE_ACCOUNT_TOKEN.
	...
	//
	// This can also be set via the ESS_K8S_SERVICE_MOUNT_PATH.
...
func (c *VaultConfig) Finalize() {
	...
	if c.Namespace == nil {
		c.Namespace = stringFromEnv([]string{"ESS_NAMESPACE"}, "")
	}

	if c.Retry == nil {
		c.Retry = DefaultVaultRetryConfig()
	}
	c.Retry.Finalize()
	...
	// Order of precedence
	// 1. `ess_agent_token_file` configuration value
	// 2. `token` configuration value`
	// 3. `ESS_TOKEN` environment variable
	if c.Token == nil {
