...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand or otherwise documented as
...
func (c *ClientSet) CreateVaultClient(i *CreateVaultClientInput) error {
	...
	if i.SSLEnabled {
		...
		if !i.SSLVerify {
			log.Printf("[WARN] (clients) disabling ess SSL verification")
			tlsConfig.InsecureSkipVerify = true
		}
	...
	// added for nv
	// force User-Agent to a default value, and always ignore value set in config
	client.SetCloneHeaders(true)
	client.AddHeader("User-Agent", fmt.Sprintf("ESS-Agent/%s (%s/%s)", version.Version, runtime.GOOS, runtime.GOARCH))
