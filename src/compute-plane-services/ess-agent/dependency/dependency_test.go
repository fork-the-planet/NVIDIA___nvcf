...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand and CI/integration hardening or otherwise documented as
...
func TestMain(m *testing.M) {
	...
	if os.Getenv("SKIP_INTEGRATION_TESTS") != "" {
		fmt.Fprintln(os.Stderr, "SKIP_INTEGRATION_TESTS set — skipping dependency integration tests")
		os.Exit(0)
	}
...
func runTestVault() {
	...
	// Use a dynamic port to avoid conflicts with CI vault sidecars on 8200
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		Fatalf("failed to find free port for vault: %v", err)
	}
	port := listener.Addr().(*net.TCPAddr).Port
	listener.Close()

	listenAddr := fmt.Sprintf("127.0.0.1:%d", port)
	vaultAddr = fmt.Sprintf("http://%s", listenAddr)
	...
	// Wait for vault to be ready — dev server needs time to unseal and
	// initialize the root token before it will accept authenticated requests.
	healthURL := fmt.Sprintf("http://%s/v1/sys/health", listenAddr)
	for i := 0; i < 50; i++ {
	...
	Fatalf("vault did not become ready at %s", vaultAddr)
}
