...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling Vault mount/path handling or otherwise documented as
...
var (
	...
	// added for nv
	nonKVv2PathPrefixes = []string{"auth/", "sys/"}
)
...
func renewSecret(clients *ClientSet, d renewer) error {
	...
	// added for nv
	// log secret path with lease id
	if vaultSecret != nil {
		log.Printf("[INFO] %s starting renewer: lease_id=%s", d.String(), vaultSecret.LeaseID)
	}
...
func leaseCheckWait(s *Secret) time.Duration {
	...
	if vaultSecretRenewable(s) {
		...
		// added for nv
		// secret wait duration should be between 78% and 82% of total secret lease
		// nv TODO integrate VaultLeaseRenewalThreshold
		sleep = float64(time.Duration(base)*time.Second) * (.80 + ((rand.Float64()*4)-2)*0.01)
	}
...
func isKVv2(client *api.Client, path string) (string, bool, error) {
	// added for nv
	// check if path has either auth/ or sys/ as a prefix
	// if so, skip isKVv2 check
	trimmedPath := strings.TrimPrefix(path, "/")
	for _, prefix := range nonKVv2PathPrefixes {
	...
	if err != nil {
		...
		//
		// added for nv
		// if status code is 404, then return error instead of assuming it's KV v1
		//
