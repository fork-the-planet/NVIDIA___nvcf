...
var _ NvDependency = (*VaultListQuery)(nil)
...
// added for nv
type NvVaultListQueryInput struct {
	SecretUrl                   string
...
// added for nv
func NewNvVaultListQuery(i *NvVaultListQueryInput) (*VaultListQuery, error) {
	query, err := NewVaultListQuery(i.SecretUrl)
	if err != nil {
		return nil, err
	}
...
func (d *VaultListQuery) Fetch(clients *ClientSet, opts *QueryOptions) (interface{}, *ResponseMetadata, error) {
	...
	if err != nil {
		// added for nv
		return nil, nil, shimNvClientError(d.nv, d.path, errors.Wrap(err, d.String()))
	}
...
func (d *VaultListQuery) String() string {
	return fmt.Sprintf("ess.list(%s)", d.path)
}
...
func (d *VaultListQuery) IsRunOnce() bool {
	return d.nv.runOnce
}

// added for nv
// TemplateID captures the template associated with the dependency
func (d *VaultListQuery) TemplateID() string {
