...
var (
	...
	// added for nv
	// added for substituting in test cases
...
func AtomicWrite(path string, createDestDirs bool, contents []byte, perms os.FileMode, backup bool) error {
	...
	//	log.Printf("[WARN] (renderer) could not file ownership for path %s because feature has not been implemented for Wimdows at this time.", path)
	//} else {
	//	if err := chownFile(f, user, group); err != nil {
	//		log.Printf("[ERROR] (renderer) could not chown file %s: %v", path, err)
	//	}
	//}
...
// added for nv
// nv TODO validate opens source implementation of file ownership

//func chownFile(file *os.File, userName *string, groupName *string) error {
//	if isStrPtrEmptyOrNil(userName) && isStrPtrEmptyOrNil(groupName) {
//		return nil
//	}
//	if isStrPtrEmptyOrNil(userName) {
//		return fmt.Errorf("user is empty or nil; must have both user and group")
//	}
//	if isStrPtrEmptyOrNil(groupName) {
//		return fmt.Errorf("group is empty or nil; must have both user and group")
//	}
//
//	// Get uid of userName
//	var uid int
//	if userInfo, err := lookupUser(*userName); err != nil {
//		return fmt.Errorf("could not get uid for user %s: %v", *userName, err)
//	} else {
//		if uid, err = strconv.Atoi(userInfo.Uid); err != nil {
//			return fmt.Errorf("could not convert uid %s into an int: %v", userInfo.Uid, err)
//		}
//	}
//
//	// Get gid of groupName
//	var gid int
//	if groupInfo, err := lookupGroup(*groupName); err != nil {
//		return fmt.Errorf("could not get gid for group %s: %v", *groupName, err)
//	} else {
//		if gid, err = strconv.Atoi(groupInfo.Gid); err != nil {
//			return fmt.Errorf("could not convert gid %s into an int: %v", groupInfo.Gid, err)
//		}
//	}
//
//	if err := chown(file.Name(), uid, gid); err != nil {
//		return fmt.Errorf("could not chown file to user %q and group %q: %v", *userName, *groupName, err)
//	}
//
//	return nil
//}
...
// added for nv
func isStrPtrEmptyOrNil(s *string) bool {
	if s == nil {
		return true
	}
	if *s == "" {
