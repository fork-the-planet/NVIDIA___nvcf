...
func pkiCertFunc(b *Brain, used, missing *dep.Set, destPath string, nv NvFuncMapInput) func(...string) (interface{}, error) {
	return func(s ...string) (interface{}, error) {
		if len(s) == 0 {
			return nil, nil
		}
		...
		// added for nv
		// use NewNvVaultPKIQuery() wrapper function to init dependency
		d, err := dep.NewNvVaultPKIQuery(&dep.NvVaultPKIQueryInput{
			UrlPath:                     path,
			FilePath:                    destPath,
			Data:                        data,
			ErrorOnMissingKey:           nv.errorOnMissingKey,
			RunOnce:                     nv.runOnce,
			SkipMountVersionCheck:       nv.skipMountVersionCheck,
			ExitOnClientError:           nv.exitOnClientError,
			StopProcessingOnClientError: nv.stopProcessingOnClientError,
			Destination:                 nv.destination,
			TemplateID:                  nv.templateID,
		})
		if err != nil {
			return nil, err
		}
...
func secretFunc(b *Brain, used, missing *dep.Set, nv NvFuncMapInput) func(...string) (interface{}, error) {
	return func(s ...string) (interface{}, error) {
		...
		// added for nv
		// modified to use "NewNvVaultReadQuery" and "NewNvVaultWriteQuery"
		isReadQuery := len(rest) == 0
		if isReadQuery {
			d, err = dep.NewNvVaultReadQuery(&dep.NvVaultReadQueryInput{
				SecretUrl:                   path,
				ErrorOnMissingKey:           nv.errorOnMissingKey,
				RunOnce:                     nv.runOnce,
				SkipMountVersionCheck:       nv.skipMountVersionCheck,
			...
			d, err = dep.NewNvVaultWriteQuery(&dep.NvVaultWriteQueryInput{
				SecretUrl:                   path,
				Data:                        data,
				ErrorOnMissingKey:           nv.errorOnMissingKey,
				RunOnce:                     nv.runOnce,
...
func secretsFunc(b *Brain, used, missing *dep.Set, nv NvFuncMapInput) func(string) ([]string, error) {
	return func(s string) ([]string, error) {
		var result []string
		...
		d, err := dep.NewNvVaultListQuery(&dep.NvVaultListQueryInput{
			SecretUrl:                   s,
			SkipMountVersionCheck:       nv.skipMountVersionCheck,
			ExitOnClientError:           nv.exitOnClientError,
			StopProcessingOnClientError: nv.stopProcessingOnClientError,
