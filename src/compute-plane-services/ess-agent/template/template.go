...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS Vault template behavior or otherwise documented as
...
// start for nv
// https://github.com/hashicorp/consul-template/issues/2031
var (
...
type Template struct {
	...
	// added for nv
	// renderOnce determines if a template should only be rendered once
	renderOnce bool
...
func NewTemplate(i *NewTemplateInput) (*Template, error) {
	...
	// added for nv
	t.renderOnce = i.RenderOnce
	t.skipMountVersionCheck = i.SkipMountVersionCheck
	t.exitOnClientError = i.ExitOnClientError
	t.stopProcessingOnClientError = i.StopProcessingOnClientError
...
// IsOnce returns the bool that determines if a template is to be rendered once.
// added for nv
func (t *Template) IsOnce() bool {
	return t.renderOnce
}

// SkipMountVersionCheck returns the bool that determines if the secret path(s) defined in the template
// should be validated via sys/internal/ui/mounts before fetching secrets.
//
// added for nv
func (t *Template) SkipMountVersionCheck() bool {
	return t.skipMountVersionCheck
}

// IsExitOnClientErrorEnabled returns the bool that determines if process should exit if the secret path(s) defined in the template
// return a 40x status code
//
// added for nv
func (t *Template) IsExitOnClientErrorEnabled() bool {
	return t.exitOnClientError
}

// IsStopProcessingOnClientErrorEnabled returns the bool that determines if template processing should stop (but keep agent running)
// if the secret path(s) defined in the template return a 40x status code
//
// added for nv
func (t *Template) IsStopProcessingOnClientErrorEnabled() bool {
	return t.stopProcessingOnClientError
}
...
func (t *Template) Execute(i *ExecuteInput) (*ExecuteResult, error) {
	...
	tmpl.Funcs(funcMap(&funcMapInput{
		...
		// added for nv
		nv: NvFuncMapInput{
			errorOnMissingKey:           t.errMissingKey,
			runOnce:                     t.IsOnce(),
			skipMountVersionCheck:       t.SkipMountVersionCheck(),
			exitOnClientError:           t.IsExitOnClientErrorEnabled(),
			destination:                 t.destination,
			templateID:                  t.ID(),
			stopProcessingOnClientError: t.IsStopProcessingOnClientErrorEnabled(),
		},
	}))
...
func redactinator(used *dep.Set, b *Brain, err error) error {
	...
	return fmt.Errorf("%s", strings.NewReplacer(pairs...).Replace(err.Error()))
}
...
func funcMap(i *funcMapInput) template.FuncMap {
	...
	r := template.FuncMap{
		...
		"secret":       secretFunc(i.brain, i.used, i.missing, i.nv),
		"secrets":      secretsFunc(i.brain, i.used, i.missing, i.nv),
		...
		// added for nv
		// add "nv" struct for custom parameters
		"pkiCert": pkiCertFunc(i.brain, i.used, i.missing, i.destination, i.nv),
