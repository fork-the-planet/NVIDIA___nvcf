...
Not a contribution
Changes made by NVIDIA CORPORATION & AFFILIATES enabling ESS agent rebrand and telemetry or otherwise documented as
...
func VaultTokenWatcher(
	...
	// added for nv
	// mark ess_last_token_file_refresh_unix metric to current time as we just read the token from the file.
	telemetry.SetLastTokeFileRefreshTimestamp(time.Now().Unix())
...
func watchTokenFile(
	...
	if err != nil {
		return nil, fmt.Errorf("esswatcher: %w", err)
	}
		...
		for {
			...
			case v := <-w.DataCh():
				...
				case nil:
					...
					vault.SetToken(token)
					// added for nv
					// all updates to the token file should be captured
					telemetry.SetLastTokeFileRefreshTimestamp(int64(v.lastIndex))
				default:
...
func unpackToken(client vaultClient, token string, unwrap bool) (string, error) {
	...
	if unwrap {
		...
		switch {
		case err != nil:
			return token, fmt.Errorf("ess unwrap: %s", err)
		case secret == nil:
			return token, fmt.Errorf("ess unwrap: no secret")
		case secret.Auth == nil:
			return token, fmt.Errorf("ess unwrap: no secret auth")
		case secret.Auth.ClientToken == "":
			return token, fmt.Errorf("ess unwrap: no token returned")
		default:
