...
func (v *View) poll(viewCh chan<- *View, errCh chan<- error) {
	...
	WAIT:
		...
		case <-successCh:
			...
			// added for nv
			if !nv.IsInInitMode() {
				if nvDep, ok := v.Dependency().(dep.NvDependency); ok {
					operation, path := nv.ParseActionAndPath(nvDep.String())
					v.instruments.CounterTemplateSecretFetchFailures.Add(context.Background(), 1,
						v.instruments.Labels(map[string]string{
							"id":        nvDep.TemplateID(),
							"operation": operation,
							"secret":    path,
							"status":    "success",
		...
		case err := <-fetchErrCh:
			// added for nv
			// skip retry and return error if secret cannot be found in Vault and error_on_missing_key is true
			var skipRetry bool
			if err != nil {
				var clientErr *dep.NvClientError
				var noSecretErr *dep.NvNoSecretAtPathError
				var stopProcessingErr *dep.NvStopProcessingError
				if errors.As(err, &clientErr) {
					log.Printf("[ERR] (view) %s (skipping retry attempts)", clientErr)
					skipRetry = true
				} else if errors.As(err, &noSecretErr) && noSecretErr.ErrorOnMissingKeyEnabled() {
					log.Printf("[ERR] (view) %s (skipping retry attempts)", noSecretErr)
					skipRetry = true
				} else if errors.As(err, &stopProcessingErr) {
					log.Printf("[ERR] (view) %s (stop processing error, skipping retry attempts)", stopProcessingErr)
					skipRetry = true
				}
			}

			if !errors.Is(err, errLookup) && v.retryFunc != nil && !skipRetry {
			...
			// added for nv
			// on the sidecar container, if we got a 4XX or 5XX error, then we should ensure we don't let the ess-agent die.
			// swallow the error and not bubble it up i.e, think of it as success
			// EXCEPT for NvStopProcessingError which needs to reach the runner for template stopping logic
			var stopProcessingErr *dep.NvStopProcessingError
			if !nv.IsInInitMode() && !errors.As(err, &stopProcessingErr) {
				if nvDep, ok := v.Dependency().(dep.NvDependency); ok {
					operation, path := nv.ParseActionAndPath(nvDep.String())
					v.instruments.CounterTemplateSecretFetchFailures.Add(context.Background(), 1,
						v.instruments.Labels(map[string]string{
				...
				retries = 0
				log.Printf("[INFO] (view) %s next poll in %q", v.Dependency().String(), dep.VaultDefaultLeaseDuration)
				time.Sleep(dep.VaultDefaultLeaseDuration)
				continue
			}

			// Push the error back up to the watcher
			// This handles: init mode (all errors), non-init mode (only NvStopProcessingError)
			select {
			case <-v.stopCh:
				return
			case errCh <- err:
