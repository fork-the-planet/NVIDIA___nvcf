...
func main(m *testing.M) int {
	...
	if os.Getenv("SKIP_INTEGRATION_TESTS") != "" {
		fmt.Fprintln(os.Stderr, "SKIP_INTEGRATION_TESTS set — skipping watch integration tests")
		return 0
	}
...
func newTestVault() *vaultServer {
	...
	if err != nil {
		panic("failed to find free port for vault: " + err.Error())
	}
...
func runVaultAgent(clients *dep.ClientSet, role_id string) string {
	...
	if err := cmd.Run(); err != nil {
		panic("ess agent failed to run: " + err.Error())
	}
	return tokenFile
}
