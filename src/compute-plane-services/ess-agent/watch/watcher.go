...
func (w *Watcher) Add(d dep.Dependency) (bool, error) {
	...
	// added for nv
	// default once to value from watcher
	// however, if the dependency itself is in run once mode then prioritize that value instead
	once := w.once
	if nvDep, ok := d.(dep.NvDependency); ok && nvDep.IsRunOnce() {
		once = nvDep.IsRunOnce()
	}
