...
`selfManaged.transportTls` is generated by `nvcf self-hosted compute-plane register` from the reviewed control-plane profile. It carries public worker transport trust only. In `bundle` mode, provide the public CA bundle with its matching `nvcf-trust-bundle-v1` fingerprint; NVCA creates the namespace-local worker trust bundles. The chart does not render worker trust ConfigMaps or carry private keys.
...
| `selfManaged.transportTls.trustMode`          | Worker transport TLS trust mode from the control-plane profile. Use "system" for system roots or "bundle" for a pinned public trust bundle.                                        | `system`                                  |
...
| `selfManaged.transportTls.trustBundlePem`     | Public PEM trust bundle used when trustMode is "bundle". Do not put private keys or service leaf material here.                                                                  | `""`                                      |
