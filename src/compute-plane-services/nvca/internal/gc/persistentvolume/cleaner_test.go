...
func TestCleaner_Name(t *testing.T) {
	cleaner := &Cleaner{}
	assert.Equal(t, "PersistentVolumeCleaner", cleaner.Name())
}

// Test collectOrphanedPersistentVolumes method
func TestCleaner_collectOrphanedPersistentVolumes(t *testing.T) {
	tests := []struct {
		name              string
		...
		{
			name:              "no persistent volumes",
			persistentVolumes: []corev1.PersistentVolume{},
			expectedOrphaned:  0,
			expectError:       false,
		},
		{
			name: "persistent volumes without storage-request-namespace label are ignored",
			persistentVolumes: []corev1.PersistentVolume{
				{
		...
		{
			name: "collect orphaned persistent volumes - namespace missing",
			persistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv",
		...
		{
			name: "collect orphaned persistent volumes - ICMSRequest missing",
			persistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv",
		...
		{
			name:              "error listing persistent volumes",
			persistentVolumes: []corev1.PersistentVolume{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			objects := make([]runtime.Object, len(tt.persistentVolumes))
			for i, pv := range tt.persistentVolumes {
				objects[i] = &pv
			}
			...
			if tt.simulateListError {
				k8sClient.PrependReactor("list", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
					return true, nil, errors.New("failed to list persistent volumes")
				})
			}
			...
			orphaned, err := cleaner.collectOrphanedPersistentVolumes(ctx)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.namespaceExists {
				nsName := tt.persistentVolume.Labels[storage.StorageRequestNamespaceKey]
				ns := &corev1.Namespace{
					ObjectMeta: metav1.ObjectMeta{
						Name: nsName,
					},
			...
			// Create cleaner and test
			cleaner := NewCleaner(k8sClient, mockICMSGetter, nil)
			result := cleaner.isOrphaned(ctx, tt.persistentVolume)
...
// Test cleanupPersistentVolume edge cases
func TestCleaner_cleanupPersistentVolume_EdgeCases(t *testing.T) {
	tests := []struct {
		name                   string
		persistentVolume       *corev1.PersistentVolume
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create fake k8s client
			k8sClient := fake.NewSimpleClientset(tt.persistentVolume)

			// Setup error simulation
			if tt.simulateUpdateError {
				k8sClient.PrependReactor("update", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
					return true, nil, errors.New("update failed")
				})
			}

			if tt.simulateDeleteError {
				k8sClient.PrependReactor("delete", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
			...
			if tt.simulateAlreadyDeleted {
				k8sClient.PrependReactor("delete", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
					return true, nil, apierrors.NewNotFound(schema.GroupResource{Group: "", Resource: "persistentvolumes"}, tt.persistentVolume.Name)
				})
			}
			...
			// Create cleaner and test
			cleaner := NewCleaner(k8sClient, mockICMSGetter, nil)
			err := cleaner.cleanupPersistentVolume(ctx, tt.persistentVolume)
...
func TestCleaner_executeCleanup(t *testing.T) {
	tests := []struct {
		name                      string
		orphanedPersistentVolumes []corev1.PersistentVolume
		simulateCleanupErrors     []string // PV names that should fail cleanup
		expectNoError             bool
	}{
		{
			name:                      "no orphaned persistent volumes",
			orphanedPersistentVolumes: []corev1.PersistentVolume{},
			expectNoError:             true,
		},
		{
			name: "successful cleanup of multiple persistent volumes",
			orphanedPersistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv-1",
		...
		},
		{
			...
			orphanedPersistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv-success",
					},
				},
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv-error",
					},
				},
			},
			simulateCleanupErrors: []string{"orphaned-pv-error"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create fake k8s client
			objects := make([]runtime.Object, len(tt.orphanedPersistentVolumes))
			for i, pv := range tt.orphanedPersistentVolumes {
				objects[i] = &pv
			}
			k8sClient := fake.NewSimpleClientset(objects...)
			...
			// Create cleaner and test
			cleaner := NewCleaner(k8sClient, mockICMSGetter, nil)
			err := cleaner.executeCleanup(ctx, tt.orphanedPersistentVolumes)
...
func TestCleaner_Run_ListError(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to collect orphaned persistent volumes")
}

// Test Run method with large number of persistent volumes
func TestCleaner_Run_LargeScale(t *testing.T) {
	ctx := context.Background()

	// Create many persistent volumes
	const numPersistentVolumes = 50
	persistentVolumes := make([]runtime.Object, numPersistentVolumes)
	for i := 0; i < numPersistentVolumes; i++ {
		pv := &corev1.PersistentVolume{
			ObjectMeta: metav1.ObjectMeta{
				Name: fmt.Sprintf("orphaned-pv-%d", i),
				Labels: map[string]string{
					storage.StorageRequestNamespaceKey: fmt.Sprintf("missing-namespace-%d", i),
				},
			},
		}
		persistentVolumes[i] = pv
	}

	// Create fake k8s client
	k8sClient := fake.NewSimpleClientset(persistentVolumes...)
	...
	// Verify all persistent volumes were cleaned up
	remainingPVs, err := k8sClient.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
	require.NoError(t, err)
	assert.Equal(t, 0, len(remainingPVs.Items))
}
...
func TestCleaner_cleanupPersistentVolume(t *testing.T) {
	tests := []struct {
		name                   string
		persistentVolume       *corev1.PersistentVolume
		expectError            bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create fake k8s client with the persistent volume
			k8sClient := fake.NewSimpleClientset(tt.persistentVolume)
			...
			// Check if persistent volume was deleted
			if tt.expectDeletion {
				_, err := k8sClient.CoreV1().PersistentVolumes().Get(ctx, tt.persistentVolume.Name, metav1.GetOptions{})
				assert.True(t, apierrors.IsNotFound(err), "Persistent volume should be deleted")
			}
		})
	}
}
...
func TestCleaner_Run(t *testing.T) {
	tests := []struct {
		...
		{
			name: "one orphaned persistent volume - missing namespace",
			persistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv",
		...
		{
			name: "one orphaned persistent volume - missing ICMSRequest",
			persistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv",
		...
		{
			name: "mixed persistent volumes - some orphaned, some not",
			persistentVolumes: []corev1.PersistentVolume{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "orphaned-pv",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create fake k8s client with persistent volumes
			objects := make([]runtime.Object, len(tt.persistentVolumes))
			for i, pv := range tt.persistentVolumes {
				objects[i] = &pv
			}
			...
			// Check how many persistent volumes remain
			remainingPVs, err := k8sClient.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
			require.NoError(t, err)

			expectedRemaining := len(tt.persistentVolumes) - tt.expectedCleanupCount
			assert.Equal(t, expectedRemaining, len(remainingPVs.Items), "Unexpected number of persistent volumes remaining")
		})
	}
}

func TestCleaner_cleanupPersistentVolume_ConflictRetry(t *testing.T) {
	ctx := context.Background()

	// Persistent volume with finalizers
	pv := &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "test-pv",
			Finalizers: []string{"test-finalizer"},
	...
	k8sClient.PrependReactor("update", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
		updateAttempts++
		if updateAttempts == 1 {
			return true, nil, apierrors.NewConflict(schema.GroupResource{Group: "", Resource: "persistentvolumes"}, pv.Name, errors.New("conflict"))
		}
	...
	cleaner := NewCleaner(k8sClient, mockICMSGetter, nil)
	err := cleaner.cleanupPersistentVolume(ctx, pv)

	assert.NoError(t, err)
	assert.Equal(t, 2, updateAttempts, "PersistentVolume should have been retried on conflict")

	// Verify persistent volume was deleted
	_, err = k8sClient.CoreV1().PersistentVolumes().Get(ctx, pv.Name, metav1.GetOptions{})
	assert.True(t, apierrors.IsNotFound(err))
}

func TestCleaner_cleanupPersistentVolume_ResourceDeletedDuringRetry(t *testing.T) {
	ctx := context.Background()
	...
	var getAttempts int
	k8sClient.PrependReactor("get", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
		getAttempts++
		if getAttempts > 1 {
			return true, nil, apierrors.NewNotFound(schema.GroupResource{Group: "", Resource: "persistentvolumes"}, pv.Name)
		}
		return false, nil, nil // Let normal processing continue
	})
...
func TestCleaner_executeCleanup_DataRaceStress(t *testing.T) {
	...
	// Create a large number of orphaned persistent volumes to stress test concurrent cleanup
	const numPVs = 100
	orphanedPVs := make([]corev1.PersistentVolume, numPVs)
	objects := make([]runtime.Object, numPVs)

	for i := 0; i < numPVs; i++ {
		pv := corev1.PersistentVolume{
			ObjectMeta: metav1.ObjectMeta{
				Name: fmt.Sprintf("stress-test-pv-%d", i),
				Labels: map[string]string{
					storage.StorageRequestNamespaceKey: fmt.Sprintf("ns-%d", i),
				},
				// Mix of PVs with and without finalizers to test different code paths
	...
	k8sClient.PrependReactor("delete", "persistentvolumes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
		deleteAction := action.(k8stesting.DeleteAction)
		pvName := deleteAction.GetName()

		// Store each deletion attempt
		if _, loaded := deletionAttempts.LoadOrStore(pvName, true); loaded {
			// PV was already attempted to be deleted
			t.Errorf("PersistentVolume %s was attempted to be deleted multiple times - potential data race", pvName)
		}

		// Fail 10% of deletions randomly to test error handling
		hash := 0
		for _, c := range pvName {
			hash += int(c)
		}
		if hash%10 == 0 {
	...
	err := cleaner.executeCleanup(ctx, orphanedPVs)

	// Should always complete without error even if some deletions fail
	assert.NoError(t, err)

	// Verify that we attempted to delete each PV exactly once
	uniqueDeletions := 0
	...
	// We should have attempted to delete all PVs
	assert.Equal(t, numPVs, uniqueDeletions, "Each PV should be deleted exactly once")
}

// Test parallel cleanup of persistent volumes with complex patterns
func TestCleaner_Run_ParallelComplexOwners(t *testing.T) {
	...
	// Create a mix of persistent volumes with different ownership patterns
	for i := 0; i < numPVs; i++ {
		var labels map[string]string

		switch i % 4 {
		case 0:
			// Orphaned - namespace doesn't exist
			labels = map[string]string{
		...
		case 2:
			// Not orphaned - missing required label
			labels = nil
		case 3:
			// Not orphaned - has label but without value
			labels = map[string]string{
				"some-other-label": "value",
			}
		}
		...
		pv := &corev1.PersistentVolume{
			ObjectMeta: metav1.ObjectMeta{
				Name:   fmt.Sprintf("complex-pv-%d", i),
				Labels: labels,
			},
		}
	...
	// Verify that only PVs with label and missing namespace/ICMSRequest were cleaned up
	// That's case 0 and 1, which is 50% of PVs
	remainingPVs, err := k8sClient.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
	require.NoError(t, err)

	expectedRemaining := numPVs / 2 // Cases 2 and 3 should remain
	assert.Equal(t, expectedRemaining, len(remainingPVs.Items))
}
