...
func TestNewTokenFetcher(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			revalEndpoint: "https://stage.nvidia.com",
			envSetup:      map[string]string{},
			wantErr:       false,
		},
		{
			...
			revalEndpoint: "https://prod.nvidia.com",
			envSetup:      map[string]string{},
			wantErr:       false,
		},
		{
			...
			revalEndpoint: "https://stage.nvidia.com",
			envSetup: map[string]string{
				"HELM_REVAL_OAUTH_CLIENT_ID":         "env-client-id",
				"HELM_REVAL_OAUTH_CLIENT_SECRET_KEY": "env-secret-key",
			},
			wantErr: false,
		},
		{
			name: "with NGC API key parameters",
			opts: nvcaauth.TokenFetcherOptions{
				OAuthClientID:        "test-client-id",
				OAuthClientSecretKey: "test-secret-key",
				NGCServiceAPIKey:     "test-ngc-key",
			},
			revalEndpoint: "https://prod.nvidia.com",
			envSetup:      map[string]string{},
			wantErr:       false,
		...
		{
			name: "with NGC API key file",
			opts: nvcaauth.TokenFetcherOptions{
				OAuthClientID:        "test-client-id",
				OAuthClientSecretKey: "test-secret-key",
				NGCServiceAPIKeyFile: "/test/ngc-key-file",
			},
			revalEndpoint: "https://stage.stg.nvidia.com",
			envSetup:      map[string]string{},
			wantErr:       false,
		},
		{
			name: "environment variables override NGC options",
			opts: nvcaauth.TokenFetcherOptions{
				OAuthClientID:        "test-client-id",
				OAuthClientSecretKey: "test-secret-key",
				NGCServiceAPIKey:     "default-ngc-key",
				NGCServiceAPIKeyFile: "/default/ngc-key-file",
			},
			revalEndpoint: "https://prod.nvidia.com",
			envSetup: map[string]string{
				"HELM_REVAL_NGC_SERVICE_API_KEY":      "env-ngc-key",
				"HELM_REVAL_NGC_SERVICE_API_KEY_FILE": "/env/ngc-key-file",
			},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fetcher, healthCheck, err := NewTokenFetcher(ctx, tt.opts, tt.revalEndpoint)
			if tt.wantErr {
				assert.Error(t, err)
...
func TestGetHelmRevalTokenFetcherOptions(t *testing.T) {
	tests := []struct {
		name          string
		opts          nvcaauth.TokenFetcherOptions
		revalEndpoint string
		...
		},
		{
			...
			revalEndpoint: "https://something.stg.something-else.nvidia.com",
			envSetup:      map[string]string{},
			want: nvcaauth.TokenFetcherOptions{
				OAuthClientID:             "test-client-id",
				OAuthClientSecretKey:      "test-secret-key",
		...
		{
			name: "stage environment - stg hostname prefix",
			opts: nvcaauth.TokenFetcherOptions{
				OAuthClientID:        "test-client-id",
				OAuthClientSecretKey: "test-secret-key",
				NGCServiceAPIKey:     "test-ngc-key",
			...
			revalEndpoint: "https://stg.reval.nvidia.com",
			envSetup:      map[string]string{},
			want: nvcaauth.TokenFetcherOptions{
				OAuthClientID:             "test-client-id",
				OAuthClientSecretKey:      "test-secret-key",
		...
		},
		{
			...
			revalEndpoint: "https://stage.stg.nvidia.com",
			envSetup: map[string]string{
				"HELM_REVAL_STAGE_OAUTH_TOKEN_URL": "https://custom.stage.token.url",
				"HELM_REVAL_STAGE_OAUTH_JWKS_URL":  "https://custom.stage.jwks.url",
			},
			want: nvcaauth.TokenFetcherOptions{
				OAuthClientID:             "test-client-id",
				OAuthClientSecretKey:      "test-secret-key",
		...
		},
		{
			...
			want: nvcaauth.TokenFetcherOptions{
				...
				NGCServiceAPIKey:          "env-ngc-key",
				NGCServiceAPIKeyFile:      "/env/ngc-key-file",
			},
		},
		{
			...
			revalEndpoint: "https://prod.nvidia.com",
			envSetup: map[string]string{
				"HELM_REVAL_PROD_OAUTH_TOKEN_URL": "https://custom.prod.token.url",
				"HELM_REVAL_PROD_OAUTH_JWKS_URL":  "https://custom.prod.jwks.url",
			},
			want: nvcaauth.TokenFetcherOptions{
				OAuthClientID:             "test-client-id",
				OAuthClientSecretKey:      "test-secret-key",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := getHelmRevalTokenFetcherOptions(tt.opts, tt.revalEndpoint)
			...
			assert.Equal(t, tt.want.NGCServiceAPIKey, got.NGCServiceAPIKey)
			assert.Equal(t, tt.want.NGCServiceAPIKeyFile, got.NGCServiceAPIKeyFile)
			assert.Equal(t, tt.want.OAuthTokenFetchFailureThreshold, got.OAuthTokenFetchFailureThreshold)
		})
	}
}

func TestGetHelmRevalTokenFetcherOptions_WithEnvVars(t *testing.T) {
	...
	origNGCKeyFile := os.Getenv(ngcServiceAPIKeyFileEnvVarKey)
	origNGCKey := os.Getenv(ngcServiceAPIKeyEnvVarKey)
	...
	defer func() {
		...
		os.Setenv(ngcServiceAPIKeyFileEnvVarKey, origNGCKeyFile)
		os.Setenv(ngcServiceAPIKeyEnvVarKey, origNGCKey)
	}()
	...
	tests := []struct {
		...
		wantNGCAPIKeyFile string
		wantNGCAPIKey     string
	}{
		{
			...
			opts: nvcaauth.TokenFetcherOptions{
				...
				NGCServiceAPIKeyFile:      "/opt/ngc-key-file",
				NGCServiceAPIKey:          "opt-ngc-key",
			},
			envVars: map[string]string{
				oauthClientIDEnvVarKey:             "env-client",
				oauthClientSecretKeyEnvVarKey:      "env-secret",
				oauthClientSecretsEnvFileEnvVarKey: "/env/secrets",
				ngcServiceAPIKeyFileEnvVarKey:      "/env/ngc-key-file",
				ngcServiceAPIKeyEnvVarKey:          "env-ngc-key",
			},
			wantClientID:      "env-client",
			wantSecretKey:     "env-secret",
			wantSecretsFile:   "/env/secrets",
			wantNGCAPIKeyFile: "/env/ngc-key-file",
			wantNGCAPIKey:     "env-ngc-key",
		},
		{
			...
			envVars: map[string]string{
				...
				ngcServiceAPIKeyFileEnvVarKey:      "",
				ngcServiceAPIKeyEnvVarKey:          "",
			},
			wantClientID:      "opt-client",
			wantSecretKey:     "opt-secret",
			wantSecretsFile:   "/opt/secrets",
			wantNGCAPIKeyFile: "/opt/ngc-key-file",
			wantNGCAPIKey:     "opt-ngc-key",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := getHelmRevalTokenFetcherOptions(tt.opts, "https://test.nvidia.com")
			...
			assert.Equal(t, tt.wantNGCAPIKeyFile, result.NGCServiceAPIKeyFile)
			assert.Equal(t, tt.wantNGCAPIKey, result.NGCServiceAPIKey)

			// Verify other fields remain unchanged
			assert.Equal(t, "helmreval:render", result.OAuthTokenScope)
