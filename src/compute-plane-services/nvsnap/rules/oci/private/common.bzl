...
# nvsnap runs amd64-only at runtime (GPU hosts are x86_64). The
# multi_arch transition still produces an `_index` target for
# pipeline-shape parity with the umbrella's macro, but it only
# contains the linux_x86_64 image. To re-enable arm64 later, add
# back "//platforms:linux_arm64" + the arm64 oci.pull use_repo
# entries in MODULE.bazel + an arm64 cross-compile toolchain.
DEFAULT_PLATFORMS = [
...
def create_oci_image(
    ...
    # nvsnap is amd64-only; hard-coded target_platform instead of a
    # select(cpu:arm64|cpu:x86_64) since the arm64 toolchain is not
    # registered.
