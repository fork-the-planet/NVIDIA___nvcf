...
func TestAppNameContext(t *testing.T) {
	expectedAppName := "test-application"
	ctx := WithAppName(context.Background(), expectedAppName)
	actualAppName := GetAppName(ctx)
	assert.Equal(t, expectedAppName, actualAppName)
}

func TestGetAppName_NoAppName(t *testing.T) {
	// Context without app name should return empty string
	ctx := context.Background()
	actualAppName := GetAppName(ctx)
...
func TestDefaultAppName(t *testing.T) {
	// DefaultAppName should return executable name
	appName := DefaultAppName()
	// Should not be empty (running from test executable)
